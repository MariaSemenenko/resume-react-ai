import { useTranslation } from 'react-i18next'
import './Footer.css'

const assetBase = 'https://dev-08.semenenko.pp.ua/wp-content/themes/libro/assets/images'
const primaryLinks = [{ key: 'About', href: '/about' }, { key: 'Blog', href: '/blog' }, { key: 'Portfolio', href: '/portfolio' }]
const secondaryLinks = [{ key: 'Contact', href: '/contact' }, { key: 'Privacy Policy', href: '#privacy' }, { key: 'Terms & Conditions', href: '#terms' }]

function FooterLinks({ links, t }) {
  const pathname = window.location.pathname.replace(/\/$/, '') || '/'
  return <ul className="footer-links">{links.map((link) => {
    const active = link.href === '/blog' ? pathname === '/blog' || pathname.startsWith('/blog/') : pathname === link.href
    return <li key={link.key}><a className={active ? 'is-active' : undefined} href={link.href} aria-current={active ? 'page' : undefined}>{t(link.key)}</a></li>
  })}</ul>
}

export default function Footer() {
  const { t } = useTranslation()
  const subscribe = (event) => event.preventDefault()

  return <footer className="site-footer">
    <div className="page-container footer-container">
      <div className="footer-grid">
        <section className="footer-intro">
          <a className="footer-logo" href="#home" aria-label={t('Go to homepage')}><img src={`${assetBase}/flower.png`} alt="" /></a>
          <p>{t('When do they work well, and when do they rely on us? And finally, when do we actually need them?')}</p>
          <address className="footer-contacts">
            <a href="tel:+380671537306"><img src={`${assetBase}/phone_svg.svg`} alt="" />(+380) 67 153 73 06</a>
            <a href="mailto:Mashyni92@gmail.com"><img src={`${assetBase}/email_svg.svg`} alt="" />Mashyni92@gmail.com</a>
            <p><img src={`${assetBase}/location_svg.svg`} alt="" />{t('Cherkasy region, Ukraine, UA')}</p>
          </address>
        </section>
        <nav className="footer-nav" aria-label={t('Footer navigation')}><p className="footer-heading">{t('Useful links')}</p><FooterLinks links={primaryLinks} t={t} /></nav>
        <nav className="footer-nav" aria-label={t('Legal navigation')}><p className="footer-heading">{t('Information')}</p><FooterLinks links={secondaryLinks} t={t} /></nav>
      </div>
      <div className="footer-bottom">
        <p>© 2026 | {t('All rights reserved by')} <a href="#portfolio">{t('Web Developer')}</a></p>
        <form className="footer-form" onSubmit={subscribe}>
          <label className="sr-only" htmlFor="footer-email">{t('Email address')}</label>
          <input id="footer-email" type="email" placeholder={t('Enter your email')} autoComplete="email" required />
          <button type="submit">{t('Send')} <span aria-hidden="true">→</span></button>
        </form>
      </div>
    </div>
  </footer>
}
